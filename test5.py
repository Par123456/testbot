...
def get_support_text():
    with db_lock:
        ...
        row = cur.fetchone()
        return row[0] if row else 'پشتیبانی: تماس با @admin'
...
def get_guide_text():
    with db_lock:
        ...
        row = cur.fetchone()
        return row[0] if row else 'راهنما: دعوت دوستان برای امتیاز'
...
def main_menu():
    markup = InlineKeyboardMarkup(row_width=1)  # Increased button size by setting row_width to 1
    markup.add(InlineKeyboardButton("📩 دعوت دوستان", callback_data='invite'))
    markup.add(InlineKeyboardButton("⭐ امتیازات من", callback_data='scores'))
    markup.add(InlineKeyboardButton("ℹ️ راهنما", callback_data='guide'))
    markup.add(InlineKeyboardButton("💰 برداشت", callback_data='withdraw'))
...
def callback_handler(call):
    ...
    if data == 'invite':
        ...
        with db_lock:
            ...
            row = cur.fetchone()
            username, name = row if row else (None, None)
        ...
        if is_member_in_all(user_id):
            with db_lock:
                ...
                row = cur.fetchone()
                inviter_id, credited = row if row else (None, 0)
...
def chat_member_update(update):
    ...
    if new_status in ['left', 'kicked', 'banned']:  # Expanded to handle more leave scenarios
            ...
            if cur.fetchone():
                ...
                if row:
                    ...
                    if inviter_id and credited == 1:
                        ...
                        conn.commit()
                        try:
                            bot.send_message(inviter_id, f"زیرمجموعه شما @{username} از کانال خارج شد. امتیاز {score_ded} کسر شد.")
                        except Exception as e:
...
def text_handler(message):
    ...
    if state == 'broadcast':
        ...
        try:
            ...
            bot.send_message(user_id, "آیدی عددی معتبر وارد کنید.")
        except Exception as e:
            bot.send_message(user_id, "خطا در افزودن ادمین.")
            logging.error(f"Error adding admin {text}: {e}")
        del user_states[user_id]
        bot.send_message(user_id, "بازگشت به پنل.", reply_markup=admin_menu(user_id))
        ...
        try:
            ...
            bot.send_message(user_id, "خطا در حذف ادمین.")
            logging.error(f"Error deleting admin {text}: {e}")
        del user_states[user_id]
        bot.send_message(user_id, "بازگشت به پنل.", reply_markup=admin_menu(user_id))
        ...
        try:
            ...
            bot.send_message(user_id, "عدد معتبر و مثبت وارد کنید.")
        del user_states[user_id]
        bot.send_message(user_id, "بازگشت به پنل.", reply_markup=admin_menu(user_id))
        ...
        if text:
            ...
            bot.send_message(user_id, "متن خالی مجاز نیست.")
        del user_states[user_id]
        bot.send_message(user_id, "بازگشت به پنل.", reply_markup=admin_menu(user_id))
        ...
        if is_owner(user_id):
            ...
            markup = ReplyKeyboardMarkup(resize_keyboard=True)
            markup.add("لغو")
            bot.send_message(user_id, "آیدی عددی ادمین جدید را بفرستید (فقط عدد)", reply_markup=markup)
        else:
        ...
        if is_owner(user_id):
            ...
            markup = ReplyKeyboardMarkup(resize_keyboard=True)
            markup.add("لغو")
            bot.send_message(user_id, "آیدی عددی ادمین برای حذف بفرستید (فقط عدد)", reply_markup=markup)
        else:
            bot.send_message(user_id, "شما دسترسی ندارید.")
            ...
            if not requests:
                ...
                for req in requests:
                    ...
                    with db_lock:
                        ...
                        row = cur.fetchone()
                        username = row[0] if row else 'نامشخص'
